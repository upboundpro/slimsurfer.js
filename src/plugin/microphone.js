...
/**
 * Visualise microphone input in a slimsurfer instance.
 *
 * @implements {PluginClass}
 * @extends {Observer}
 * @example
 * // es6
 * import MicrophonePlugin from 'slimsurfer.microphone.js';
 *
 * // commonjs
 * var MicrophonePlugin = require('slimsurfer.microphone.js');
 *
 * // if you are using <script> tags
 * var MicrophonePlugin = window.SlimSurfer.microphone;
 *
...
export default class MicrophonePlugin {
    ...
    constructor(params, ws) {
        ...
        this._onBackendCreated = () => {
            // slimsurfer's AudioContext where we'll route the mic signal to
            this.micContext = this.slimsurfer.backend.getAudioContext();
        };
    }

    init() {
        this.slimsurfer.on('backend-created', this._onBackendCreated);
        if (this.slimsurfer.backend) {
            this._onBackendCreated();
        }
    }
    ...
    destroy() {
        ...
        this.slimsurfer.un('backend-created', this._onBackendCreated);
        this.stop();
    }
    ...
    reloadBuffer(event) {
        if (!this.paused) {
            this.slimsurfer.empty();
            this.slimsurfer.loadDecodedBuffer(event.inputBuffer);
        }
    }
