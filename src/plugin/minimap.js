 ...
 * @desc Extends the `WavesurferParams` slimsurfer was initialised with
 ...
 * @example
 * // es6
 * import MinimapPlugin from 'slimsurfer.minimap.js';
 *
 * // commonjs
 * var MinimapPlugin = require('slimsurfer.minimap.js');
 *
 * // if you are using <script> tags
 * var MinimapPlugin = window.SlimSurfer.minimap;
 *
...
export default class MinimapPlugin {
    ...
    constructor(params, ws) {
         ...
         *
         * @type {string}
         * @see https://github.com/katspaugh/slimsurfer.js/issues/736
         */
        this.renderEvent =
        ...
        this._onShouldRender = () => {
            ...
            if (this.slimsurfer.regions && this.params.showRegions) {
                this.regions();
            }
            this.render();
        };
        ...
        this._onAudioprocess = currentTime => {
            this.drawer.progress(this.slimsurfer.backend.getPlayedPercents());
        };
        ...
        this._onResize = ws.util.debounce(() => {
            if (prevWidth != this.drawer.wrapper.clientWidth) {
                ...
                this.drawer.progress(
                    this.slimsurfer.backend.getPlayedPercents()
                );
            }
        });
    }
    ...
    init() {
        ...
        this.slimsurfer.on(this.renderEvent, this._onShouldRender);
    }

    destroy() {
        window.removeEventListener('resize', this._onResize, true);
        window.removeEventListener('orientationchange', this._onResize, true);
        this.slimsurfer.drawer.wrapper.removeEventListener(
            'mouseover',
            this._onMouseover
        );
        this.slimsurfer.un(this.renderEvent, this._onShouldRender);
        this.slimsurfer.un('seek', this._onSeek);
        this.slimsurfer.un('scroll', this._onScroll);
        this.slimsurfer.un('audioprocess', this._onAudioprocess);
        this.drawer.destroy();
        this.overviewRegion = null;
        this.unAll();
    }
    ...
    regions() {
        ...
        this.slimsurfer.on('region-created', region => {
            this.regions[region.id] = region;
            this.renderRegions();
        });

        this.slimsurfer.on('region-updated', region => {
            this.regions[region.id] = region;
            this.renderRegions();
        });

        this.slimsurfer.on('region-removed', region => {
    ...
    renderRegions() {
        ...
        Object.keys(this.regions).forEach(id => {
            ...
            const width =
                this.drawer.width *
                ((region.end - region.start) / this.slimsurfer.getDuration());
            const left =
                this.drawer.width *
                (region.start / this.slimsurfer.getDuration());
    ...
    bindWavesurferEvents() {
        ...
        this.slimsurfer.on('audioprocess', this._onAudioprocess);
        this.slimsurfer.on('seek', this._onSeek);
        if (this.params.showOverview) {
            this.slimsurfer.on('scroll', this._onScroll);
            this.slimsurfer.drawer.wrapper.addEventListener(
                'mouseover',
                this._onMouseover
            );
        }
        ...
        if (this.params.interact) {
            ...
            this.on('click', (event, position) => {
                if (seek) {
                    ...
                    this.slimsurfer.seekAndCenter(position);
                } else {
                    seek = true;
                }
            });
    ...
    render() {
        ...
        const peaks = this.slimsurfer.backend.getPeaks(len, 0, len);
        ...
        if (this.params.showOverview) {
            ...
            this.ratio = this.slimsurfer.drawer.width / this.drawer.width;
            this.waveShowedWidth = this.slimsurfer.drawer.width / this.ratio;
            this.waveWidth = this.slimsurfer.drawer.width;
            this.overviewWidth = this.drawer.width / this.ratio;
            this.overviewPosition = 0;
            this.moveOverviewRegion(
                this.slimsurfer.drawer.wrapper.scrollLeft / this.ratio
            );
    ...
    moveOverviewRegion(pixels) {
        ...
        if (this.draggingOverview) {
            this.slimsurfer.drawer.wrapper.scrollLeft =
