...
class Init {
    ...
    constructor(SlimSurfer, params = {}) {
         ...
         * @private
         */
        const _params = (this.params = SlimSurfer.util.extend(
            {},
            {
                ...
                pluginCdnTemplate:
                    '//localhost:8080/dist/plugin/slimsurfer.[name].js',
                    ...
                    loadScript(src, { async: false }, (err, plugin) => {
                        if (err) {
                            return console.error(
                                `SlimSurfer plugin ${name} not found at ${src}`
                            );
                        }
    ...
    initEl(el, plugins = []) {
        ...
        const params = this.SlimSurfer.util.extend(
            { container: el },
        ...
        // initialise slimsurfer, load audio (with peaks if provided)
        const instance = this.SlimSurfer.create(params);
