...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		root["SlimSurfer"] = root["SlimSurfer"] || {}, root["SlimSurfer"]["html-init"] = factory();
})(this, function() {
 ...
 * mediaControls: true}] The default slimsurfer initialisation parameters
 * @property {string|NodeList} containers='slimsurfer' Selector or NodeList of
 * elements to attach instances to
 * @property {string}
 * pluginCdnTemplate='//localhost:8080/dist/plugin/slimsurfer.[name].js' URL
 ...
 *
 * The API attaches slimsurfer instances to all `<slimsurfer>` (can be
...
var Init = /*#__PURE__*/function () {
   ...
   *
   * @param {SlimSurfer} SlimSurfer The SlimSurfer library object
   * @param {InitParams} params initialisation options
   */
  function Init(SlimSurfer) {
    var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

    _classCallCheck(this, Init);

    if (!SlimSurfer) {
      throw new Error('SlimSurfer is not available!');
    }
    /**
     * cache SlimSurfer
     * @private
    ...
    this.SlimSurfer = SlimSurfer;
    /**
     * build parameters, cache them in _params so minified builds are smaller
     * @private
     */

    var _params = this.params = SlimSurfer.util.extend({}, {
      // slimsurfer parameter defaults so by default the audio player is
      ...
      containers: 'slimsurfer',
      // @TODO insert plugin CDN URIs
      pluginCdnTemplate: '//localhost:8080/dist/plugin/slimsurfer.[name].js',
      ...
      loadPlugin: function loadPlugin(name, cb) {
        ...
        (0, _loadScript.default)(src, {
          ...
          if (err) {
            return console.error("SlimSurfer plugin ".concat(name, " not found at ").concat(src));
          }

          cb(window.SlimSurfer[name]);
        });
      }
    }, params);
    /**
    ...
    this.pluginCache = {};
    /**
     * An array of slimsurfer instances
     * @type {Object[]}
     */
  ...
  _createClass(Init, [{
     ...
     *
     * @param  {HTMLElement} el The container to instantiate slimsurfer to
    ...
    value: function initEl(el) {
      ...
      var params = this.SlimSurfer.util.extend({
        container: el
      }, this.params.defaults, el.dataset, {
        plugins: initialisedPlugins
      }); // @TODO make nicer

      el.style.display = 'block'; // initialise slimsurfer, load audio (with peaks if provided)

      var instance = this.SlimSurfer.create(params);
...
if ((typeof window === "undefined" ? "undefined" : _typeof(window)) === 'object' && !window.WS_StopAutoInit) {
  ...
  if (document.readyState === 'complete') {
    window.WaveSurferInit = new Init(window.SlimSurfer, window.WS_InitOptions);
  } else {
...
//# sourceMappingURL=slimsurfer-html-init.js.map